...
BOOL TearDownGraph()
{
	...
	RemoveDownstream(pVDecom);
	RemoveDownstream(pVMR);
	SAFE_RELEASE(pCap);
	SAFE_RELEASE(pVDecom);
	if(pGraph)
	{
		RemoveALLFilters();
	}
...
extern "C" __declspec(dllexport) void ChooseDevice(int index,int flag)
{
	...
	if(!SUCCEEDED(hr))
	{
		ErrMsg(L"系统缺少必要组件，请升级到XP PS3版本!");
		return;
	}
		
	if(_EnableDecomp == 1)
	{
		SetFormat(MEDIASUBTYPE_UYVY);
		hr = CoCreateInstance(CLSID_AVIDec, NULL, CLSCTX_INPROC, IID_IBaseFilter, (void**)&pVDecom);
		hr = pGraph->AddFilter(pVDecom, L"Avi Decompressor");
		if(flag == 1)
		{
		...
		{
			hr = pBuilder->RenderStream(&PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, pCap, pVDecom, pVMR);
		}
		
		if(!SUCCEEDED(hr))
		{
			ErrMsg(L"初始化视频失败，请尝试去掉视频解码器!");
			return;
		}
	}
	else
	{
		...
		{
			hr = pBuilder->RenderStream(&PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, pCap, NULL, pVMR);
		}
		if(!SUCCEEDED(hr))
		{
			ErrMsg(L"初始化视频失败,E001");
			return;
		}
	}
	...
	if(!SUCCEEDED(hr))
	{
		ErrMsg(L"初始化视频失败,E002");
		return;
	}
	hr = pEvent->SetNotifyWindow((OAHWND)_hWindow, WM_GRAPHNOTIFY, 0);
	if(!SUCCEEDED(hr))
	{
		ErrMsg(L"初始化视频失败,E003");
		return;
	}
...
extern "C" __declspec(dllexport) bool GetVMRPRocAmpControl(VMR9ProcAmpControl *lpControl)
{
	...
	else
	{
		...
		HRESULT hr = pCap->QueryInterface(IID_IAMVideoProcAmp, (void**)&pProcAmp);
		if(SUCCEEDED(hr))
		{
			long Flags, Val;
...
extern "C" __declspec(dllexport) bool GetVMRBrightnessRange(VMR9ProcAmpControlRange *lpControlRange)
{
	...
	else
	{
		...
		if(SUCCEEDED(hr))
		{
			...
			&Default, &Flags);
			lpControlRange->StepSize = Step;
			lpControlRange->dwSize = sizeof(VMR9ProcAmpControlRange);
			lpControlRange->MaxValue = Max;
			lpControlRange->MinValue = Min;
			lpControlRange->DefaultValue = Default;
			lpControlRange->dwProperty = ProcAmpControl9_Brightness;
			return true;
		}
...
extern "C" __declspec(dllexport) bool GetVMRHueRange(VMR9ProcAmpControlRange *lpControlRange)
{
	...
	else
	{
		...
		if(SUCCEEDED(hr))
		{
			long Min, Max, Step, Default, Flags, Val;
			// Get the range and default value. 
			hr = pProcAmp->GetRange(VideoProcAmp_Hue, &Min, &Max, &Step,
			&Default, &Flags);
			lpControlRange->StepSize = Step;
			lpControlRange->dwSize = sizeof(VMR9ProcAmpControlRange);
			lpControlRange->MaxValue = Max;
...
extern "C" __declspec(dllexport) bool SetVMRPRocAmpControl(float value, int flag)
{
	...
	else
	{
		...
		if(SUCCEEDED(hr))
		{
			...
			{
			case 1:
				pProcAmp->Set(VideoProcAmp_Brightness,value,0X0002);
				break;
			case 2:
				pProcAmp->Set(VideoProcAmp_Contrast,value,0X0002);
				break;
			case 3:
				pProcAmp->Set(VideoProcAmp_Hue,value,0X0002);
				break;
			case 4:
				pProcAmp->Set(VideoProcAmp_Saturation,value,0X0002);
				break;
			}
			return true;
		}
...
extern "C" __declspec(dllexport) bool GetVideoPostion(REFERENCE_TIME *pNow,REFERENCE_TIME  *pStop)
{
	 ...
	 if(pGraph)
	 {
		 ...
		 if(SUCCEEDED(hr))
		 {
			 ...
			 if (AM_SEEKING_CanSeekAbsolute & dwCap)
			 {
				hr = pSeek->GetPositions(pNow,pStop);
			 }
			 SAFE_RELEASE(pSeek);
		 }
	 }
...
extern "C"  __declspec(dllexport) void SetDecompressor(int flag)
{
	_EnableDecomp = flag;
}